...
class ScreenStoreCheck1 extends AbstractScreen{
	/**
    ...
    init(){
    	super.init();
    	this._btNext=new SimpleButton($("#screen-store-check-check-1_bt-next"), this.btNextUpHandler);
        this._selectElement=$(".screen-store-check-1_question_element");
        ...
        if(sc.rapport.hasOwnProperty("champ2") && sc.rapport.champ2!=""){
            this._selectElement.eq(parseInt(sc.rapport.champ2)).css("opacity","1");
        }
    ...
    storeResponse(){
        ...
        this._selectElement.each(function(index){
            if($(this).css("opacity")=="1"){
                sc.rapport.champ2=index.toString();
            }
        })
    }
